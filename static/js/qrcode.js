/* 轻量 QR 码生成器 (MIT 风格自实现, 无外部依赖)
   暴露 window.qrcode(typeNumber, errorCorrectionLevel) */
(function (global) {
  "use strict";
  ...
  (function () {
    ...
    QRUtil.gexp = function (n) {
      while (n < 0) n += 255;
      while (n >= 255) n -= 255;
      return EXP_TABLE[n];
    };
    QRUtil.glog = function (n) {
  ...
  var RS_BLOCK_TABLE = [
    ...
    [4, 39, 13, 1, 40, 14], [2, 121, 97], [2, 60, 38, 2, 61, 39], [4, 40, 18, 2, 41, 19],
    [4, 40, 14, 2, 41, 15], [2, 146, 116], [3, 58, 36, 2, 59, 37], [4, 36, 16, 4, 37, 17],
    ...
    [5, 54, 24, 7, 55, 25], [1, 110, 86, 5, 109, 85], [5, 122, 98, 1, 123, 99], [7, 73, 45, 3, 74, 46],
    ...
    [2, 139, 111, 7, 140, 112], [2, 83, 55, 13, 84, 56], [7, 54, 24, 16, 55, 25],
    [34, 37, 13], [4, 151, 121, 5, 152, 122], [4, 75, 47, 14, 76, 48], [11, 54, 24, 14, 55, 25],
    [16, 45, 15, 14, 46, 16], [6, 147, 117, 4, 148, 118], [6, 73, 45, 14, 74, 46],
    [11, 54, 24, 16, 55, 25], [30, 46, 16, 2, 47, 17], [8, 132, 106, 4, 133, 107],
    [8, 75, 47, 13, 76, 48], [7, 54, 24, 22, 55, 25], [22, 45, 15, 13, 46, 16],
    [10, 142, 114, 2, 143, 115], [19, 74, 46, 4, 75, 47], [28, 50, 22, 6, 51, 23],
    [33, 46, 16, 4, 47, 17], [8, 152, 122, 4, 153, 123], [22, 73, 45, 3, 74, 46],
    [8, 53, 23, 26, 54, 24], [12, 45, 15, 28, 46, 16], [3, 147, 117, 10, 148, 118],
    [3, 73, 45, 23, 74, 46], [4, 54, 24, 31, 55, 25], [11, 45, 15, 31, 46, 16],
    [7, 146, 116, 7, 147, 117], [21, 73, 45, 7, 74, 46], [1, 53, 23, 37, 54, 24],
    [19, 45, 15, 26, 46, 16], [5, 145, 115, 10, 146, 116], [19, 75, 47, 10, 76, 48],
    [15, 54, 24, 25, 55, 25], [23, 45, 15, 25, 46, 16], [13, 145, 115, 3, 146, 116],
    [2, 74, 46, 29, 75, 47], [42, 54, 24, 1, 55, 25], [23, 45, 15, 28, 46, 16],
    ...
    [10, 45, 15, 67, 46, 16], [19, 148, 118, 6, 149, 119], [18, 75, 47, 31, 76, 48],
    [34, 54, 24, 34, 55, 25], [20, 45, 15, 61, 46, 16],
  ];

  QRUtil.getRSBlocks = function (typeNumber, errorCorrectionLevel) {
  ...
  QRUtil.getRsBlockTable = function (typeNumber, errorCorrectionLevel) {
  ...
  var FORMAT_BITS = {
    L: { 0: "111011111000100", 1: "111001011110011", 2: "111110110101010", 3: "111100010011101",
         4: "110011000101111", 5: "110001100011000", 6: "110110001000001", 7: "110100101110110" },
    M: { 0: "101010000010010", 1: "101000100100101", 2: "101111001111100", 3: "101101101001011",
         4: "100010111111001", 5: "100000011001110", 6: "100111110010111", 7: "100101010100000" },
    Q: { 0: "011010101011111", 1: "011000001101000", 2: "011111100110001", 3: "011101000000110",
         4: "010010010110100", 5: "010000110000011", 6: "010111011011010", 7: "010101111101101" },
    H: { 0: "001011010001001", 1: "001001110111110", 2: "001110011100111", 3: "001100111010000",
  ...
  QRCodeModel.prototype = {
    ...
    makeImpl: function (test, maskPattern) {
      ...
      if (this.dataCache === null) this.dataCache = createData(
        this.typeNumber, this.errorCorrectionLevel, this.dataList);
  ...
  function QRCodeFactory(typeNumber, errorCorrectionLevel) {
    ...
    if (typeNumber === 0) {
      ...
      return {
        _auto: true,
        _list: [],
        addData: function (data) { this._list.push(data); },
        make: function () {
          for (var t = 1; t <= 40; t++) {
            try {
              var qr = new QRCodeModel(t, errorCorrectionLevel);
              for (var i = 0; i < this._list.length; i++) qr.addData(this._list[i]);
              qr.make();
              this._qr = qr;
              return;
            } catch (e) {
        ...
        isDark: function (r, c) { return this._qr.isDark(r, c); },
        getModuleCount: function () { return this._qr.getModuleCount(); },
      };
    }
